...
Der Shop-Service trägt die vom vorhandenen SSH-Piper verwendeten Labels. Coolify stellt dafür zusätzlich `SERVICE_FQDN_SHOP` bereit; dessen Wert dient als externer SSH-Piper-Benutzername. Im Container wird auf `dockware:22` im dynamischen `${COMPOSE_PROJECT_NAME}`-Netzwerk weitergeleitet. Coolify benennt dieses gemeinsame Zugangsnetz wie die jeweilige Ressourcen-UUID; `_backend` bleibt intern und wird nicht für SSH verwendet. Der Aufruf entspricht damit den normalen Dev-Shops: `ssh <Clone-FQDN>@<SSH-Piper-Host>`. Das Passwort kommt ausschließlich aus `SSH_PASSWORD` der Coolify-Ressource und wird nicht ins Image geschrieben.
